...
* **5. Falsos Positivos (Nota de Escopo):**
    * Esta métrica está **fora do escopo** desta simulação. Falsos Positivos são erros de *lógica* (ex: uma regra mal escrita que classifica tráfego legítimo como ataque).
    * Nosso modelo investiga erros de *exaustão de recursos* (saturação). O descarte de um pacote legítimo (medido pelo Goodput e Latência) não é um Falso Positivo, mas sim um dano colateral de QoS causado pela saturação.
