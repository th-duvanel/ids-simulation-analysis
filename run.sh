...
echo "estudo,cenario,taxa_pps_total,taxa_bps_total,taxa_descarte_geral,taxa_deteccao_ataque" > $OUTPUT_FILE
...
for pps_por_cliente in $(seq 1000 1000 10000); do
    for size in "${PACKET_SIZES[@]}"; do
        ...
        echo "Rodando Estudo 1 ($cenario) | pps_total=$taxa_total_pps | fila=$TAMANHO_FILA"

        for i in $SEMENTES; do
            # Cenário de Gargalo (Usa NIDS_CAPACITY)
            RESULTADO=$($NS3_RUN "$SIM_SCRIPT --taxaPpsFundo=$pps_por_cliente --semente=$i --tamanhoFila=${TAMANHO_FILA}p --packetSizeFundo=$size --nidsDataRate=$NIDS_CAPACITY" | grep "RESULT")
            ...
            RESULTADO_BASE=$($NS3_RUN "$SIM_SCRIPT --taxaPpsFundo=$pps_por_cliente --semente=$i --tamanhoFila=$TAMANHO_FILA_BASELINE --packetSizeFundo=$size --nidsDataRate=$CAPACIDADE_BASELINE" | grep "RESULT")
...
for pps_por_cliente in $(seq 1000 1000 10000); do
    for fila in "${TAMANHOS_FILA[@]}"; do
        ...
        for i in $SEMENTES; do
            # Cenário de Gargalo (Usa NIDS_CAPACITY)
            RESULTADO=$($NS3_RUN "$SIM_SCRIPT --taxaPpsFundo=$pps_por_cliente --semente=$i --tamanhoFila=${fila}p --packetSizeFundo=$PACKET_SIZE_FIXO --nidsDataRate=$NIDS_CAPACITY" | grep "RESULT")
...
for pps_por_cliente in $(seq 1000 1000 10000); do
    ...
    for i in $SEMENTES; do
        # Cenário Baseline (Usa capacidade e fila "infinitas")
        RESULTADO_BASE=$($NS3_RUN "$SIM_SCRIPT --taxaPpsFundo=$pps_por_cliente --semente=$i --tamanhoFila=$TAMANHO_FILA_BASELINE --packetSizeFundo=$PACKET_SIZE_FIXO --nidsDataRate=$CAPACIDADE_BASELINE" | grep "RESULT")
